...
def read_leica_lof(lof_file_path, include_xmlelement=False):
    ...
    with open(lof_file_path, "rb") as f:
        ...
        text_length_header = struct.unpack("<i", pHeader[1:5])[
            0
        ...
        text_length = struct.unpack("<i", pXMLMem[1:5])[
            0
        ]  # This is the XML text length
    ...
    timestamp_list_element = xml_root.find(
        ".//TimeStampList"
    )  # Find TimeStampList anywhere
    ...
    if experiment_datetime is None:
        try:
            ...
            experiment_datetime = datetime.fromtimestamp(
                ctime_timestamp, tz=timezone.utc
            )
        except OSError:
            experiment_datetime = None  # Handle file system errors

    # Format the datetime string (ISO 8601 format)
    experiment_datetime_str = (
        experiment_datetime.isoformat() if experiment_datetime else None
    )
