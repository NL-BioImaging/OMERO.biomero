...
def read_leica_xlef(file_path, folder_uuid=None):
    ...
    if os.path.exists(file_path):
        try:
            ...
            root = tree.getroot()
            root_experiment_name, root_experiment_datetime = (
                _extract_experiment_details(root)
            )
        except Exception:
            pass  # Ignore if root file cannot be parsed

    if folder_uuid is None:
        result_dict = parse_top_level(
            file_path, root_experiment_name, root_experiment_datetime
        )
    else:
        result_dict = bfs_find_uuid(
            file_path, folder_uuid, root_experiment_name, root_experiment_datetime
        )
...
def bfs_find_uuid(
    top_file, folder_uuid, root_experiment_name, root_experiment_datetime
):
    """
    ...
    top_element, top_refs, top_root = parse_file_minimal(
        top_file
    )  # Modified to return root
    ...
    if folder_uuid and top_uuid == folder_uuid:
        return build_tree_for_element(
            top_ext,
            top_element,
            top_file,
            top_file,
            root_experiment_name,
            root_experiment_datetime,
        )
    ...
    while queue:
        ...
        el, refs, current_root = parse_file_minimal(
            current_file
        )  # Modified to return root
        if el is None:
            continue
...
def build_tree_for_element(
    ext, element, file_path, top_file, experiment_name, experiment_datetime
):
    """
    ...
    if ext == "xlif":
        ...
        return {
            ...
            "children": _build_children_list(
                element, file_path, top_file, experiment_name, experiment_datetime
            ),  # Pass details down
        }
...
def parse_top_level(file_path, root_experiment_name, root_experiment_datetime):
    ...
    return {
        ...
        "children": _build_children_list(
            top_el, file_path, file_path, root_experiment_name, root_experiment_datetime
        ),  # Pass details down
    }


def _build_children_list(
    element, base_file, top_file, experiment_name, experiment_datetime
):
    """
    ...
    for ref in child_elem.findall("Reference"):
        ...
        metadata = get_element_metadata(
            ref_file, ref_uuid
        )  # get_element_metadata doesn't need experiment details, they come from root
        ...
        child_node = {
            ...
            "save_child_name": (
                save_child_name if metadata.get("filetype") == ".lof" else ""
