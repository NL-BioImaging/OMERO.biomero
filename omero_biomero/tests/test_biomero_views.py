...
class BiomeroViewTests(TestCase):
    ...
    def test_biomero_context_basic(self):
        ...
        with patch.dict(os.environ, env, clear=False), \
             patch("omero_biomero.biomero_views.get_react_build_file", side_effect=lambda n: f"hashed-{n}"):
        ...
        decoded = jwt.decode(
            ctx["metabase_token_monitor_workflows"], env["METABASE_SECRET_KEY"], algorithms=["HS256"], options={"verify_exp": False}
        )
    ...
    def test_biomero_missing_env_defaults(self):
        ...
        with patch.dict(os.environ, {"METABASE_SECRET_KEY": "k", "METABASE_WORKFLOWS_DB_PAGE_DASHBOARD_ID": "1", "METABASE_IMPORTS_DB_PAGE_DASHBOARD_ID": "2"}, clear=True), \
             patch("omero_biomero.biomero_views.get_react_build_file", return_value="fallback.js"):
            ctx = _raw_biomero()(None, conn=self._fake_conn())
    ...
    def test_biomero_build_file_fallback(self):
        with patch.dict(os.environ, {"METABASE_SECRET_KEY": "k", "METABASE_WORKFLOWS_DB_PAGE_DASHBOARD_ID": "3", "METABASE_IMPORTS_DB_PAGE_DASHBOARD_ID": "4"}, clear=True):
        ...
        self.assertTrue(
            ctx["main_js"].startswith("omero_biomero/assets/main.") and ctx["main_js"].endswith(".js")
